...
/**
 * ╔══════════════════════════════════════════════════════════╗
 *  DAY 7 – Mini Project 1: CLI Number Utility Tool
 *  Week 1 Capstone — combines Loops + Methods + Conditions
 * ╚══════════════════════════════════════════════════════════╝
 *
 *  Menu:
 ...
 *
 *  Design decisions:
 *    • Every feature lives in its own static method
 *    • The main loop drives the menu (do-while keeps it alive)
 *    • Input validation handled with while-loops
 *    • All logic reuses patterns from Days 3–6
 */
public class NumberUtility {
     ...
     * Algorithm: trial division up to √num (most efficient basic approach).
     */
     ...
     * Returns long to handle larger values (up to 20!).
    ...
    /**
     * Prints factorial with the full expansion string.
     * e.g.  5! = 5 × 4 × 3 × 2 × 1 = 120
     */
     ...
     * Works for negatives too (preserves the minus sign).
     *
     * Algorithm:
     *   Take last digit via % 10
    ...
    static void reverseNum(int num) {
        ...
        for (char c : digits.toCharArray()) System.out.print(c + " ");
        System.out.println();
        ...
        if (reversed > 0 && reversed <= Integer.MAX_VALUE) {
            ...
            if (revPrime) System.out.printf("  ★  %d is also PRIME!%n", reversed);
        }
    }
